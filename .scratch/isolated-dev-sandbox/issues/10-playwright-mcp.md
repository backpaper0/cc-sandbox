...
ticket 07の`/home/dev/.cache/ms-playwright`共有ボリュームをそのまま利用する。新規ボリュームはイメージ内のブラウザを初回マウント時に引き継ぎ、ticket 10以前に作成済みの空ボリュームは`bin/sandbox up`がネットワーク隔離を適用した後に`playwright install chromium`で初回だけ補完する。複数インスタンスが同時起動しても共有キャッシュへのインストールが競合しないよう`flock`で直列化した。
...
2026-08-23にmacOS/OrbStack（Linux Docker daemon 29.4.0）で再検証し、`bats --timing test/playwright_mcp.bats`が3/3 green、`bin/test-e2e`が45/45 greenで完走した。公開stdio JSON-RPC経由の`browser_navigate`、PNGスクリーンショット生成、`down/up`後のChromiumキャッシュ再利用を実Dockerで確認済み。認証済みClaude Codeの対話からツールを選択・実行する結合経路とWSL2実機は引き続き未検証。
